...
for nic in "$NIC1" "$NIC2"; do
	grep ^net.ipv4.conf.${nic}.arp_filter /etc/sysctl.conf &> /dev/null
	if [ $? != 0 ]; then
	    errmsg "Making entries in /etc/sysctl.conf"
	    (
	    echo "net.ipv4.conf.${nic}.arp_ignore = 0"
	    echo "net.ipv4.conf.${nic}.arp_filter = 1"
	    echo "net.ipv4.conf.${nic}.rp_filter = 2"
	    echo "net.ipv4.conf.${nic}.arp_announce = 2"
	    ) >> /etc/sysctl.conf
