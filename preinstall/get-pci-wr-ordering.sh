...
usage() {
	...
	Gets PCI_WR_ORDERING (and ADVANCED_PCI_SETTINGS) for Mellanox cards. Multiple
...
main() {
	...
	# Normal mode
		...
		mst start &>/dev/null || >&2 echo "$0: error starting MST, exiting"
		...
		for port in $@; do
			...
			mlxconfig -d "$device_file" query | grep -E 'ADVANCED_PCI_SETTINGS|PCI_WR_ORDERING|LINK_TYPE' | sed "s|^|$port($device_file):|g"
		done
